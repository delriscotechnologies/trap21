...
| `TRAP21_MAX_SESSION_SECONDS` | `120` | Absolute control-session lifetime enforced by an independent socket-closing watchdog |
...
| `TRAP21_MAX_VFS_DIRECTORIES` | `4096` | Maximum persisted directories beneath the virtual root |
| `TRAP21_MAX_VFS_FILES` | `8192` | Maximum persisted files beneath the virtual root, including upload placeholders |
...
Telemetry rate limits do not throttle or alter FTP responses. For each session, TRAP21 retains up to 100 `COMMAND` events and 25 failed `AUTH_ATTEMPT` events per second. Additional events are represented by `COMMANDS_SUPPRESSED` and `AUTH_ATTEMPTS_SUPPRESSED` summaries. Successful authentication attempts and upload, transfer, error, and lifecycle events bypass rate suppression. A new session is not presented with an FTP banner unless its `CONNECT` event is successfully written and flushed. Authentication summaries count SHA-256 fingerprints rather than retain the additional plaintext values. Username and password counts are each capped at 256 distinct values per window; the summary includes `distinctCountsTruncated: true` when either cap is exceeded.

Directory and file budgets are initialized from the persisted VFS after seed data is created. Counters change only after successful filesystem mutations, and directory capacity is returned after a successful `RMD`.
